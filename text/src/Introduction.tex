...
\stlcbool using inductive definitions and show how to reason inductively about
them.
...
\steven{But with a denotation in mind.}  Usually not all expressions are
typeable and un-typeable expressions are rejected. In this sense, a type system
can be seen as a \emph{syntactic restriction} on expressions. Also in some
...
Both, the partiality and the ambiguity of types, suggests a \emph{relational}
rather than a \emph{functional} assignment. Such a relation is defined in Figure
\ref{fig:intro:stlcbooltyping}. The box in the upper left corner
\framebox{\mbox{$\typing{\Gamma}{e}{\tau}$}} defines the shape and notation that
we use for the relation. In this case it's a ternary relation between a typing
context $\Gamma$, an expression $e$ and a type $\tau$.
...
the judgement $J$ below the line is the conclusion. The conclusion will always
involve the relation that is being defined.

The typing relation in Figure \ref{fig:intro:stlcbooleval} is defined using six
rules. The two rules \textsc{TTrue} and \textsc{TFalse} respectively state that
the boolean constants $\true$ and $\false$ have a boolean type. Both rules do
not have any premises. Such rules are also called \emph{axioms}. The rule
\textsc{TIf} handles the case of an \textbf{if}-expression. The three
sub-expression position contain a meta-variables $e_c$, $e_t$ and $e_e$. The
premises require that the condition $e_c$ has type boolean and the \textbf{then}
and \textbf{else} branches have the same type $\tau$. The rule then concludes
that the entire \textbf{if}-expression also has type $\tau$.
...
Different ways to define semantics of programming language. A coarse
classification
...
We use denotational semantics in Part \ref{part:mod} and small-step operational
semantics in Part \ref{part:gen}.


For our \stlcbool example we will define semantics using a small-step
operational semantics. Figure \ref{fig:intro:stlcbooleval} gives the complete
definition by means of an evaluation relation \framebox{\mbox{$\eval{e_1}{e_2}$}}
between two terms, which denotes that $e_1$ evaluates to $e_2$ in one step.
