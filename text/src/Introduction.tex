...
\begin{proof}
  ...
  each possible case we have to inspect the last rule that was used to
  derive $\eval{e_1}{e_3}$ which will in fact be the same one that was used to
  derive $\eval{e_1}{e_2}$ and hence the property follows.

  We make this more precise for one of the inductive steps. Consider the case
  $e_1 = \ite{\true}{e_{11}}{e_{12}}$ and $\eval{e_1}{e_2}$ was derived using
  rule \textsc{EIfTrue}. Now look at the last rule that was used to derive
  $\eval{e_1}{e_3}$. There are two possibilities: rule \textsc{EIfTrue} and
  \textsc{EIf}.

  \begin{enumerate}
  \item If rule \textsc{EIfTrue} was used, then we have the chain of equalities
    $e_2 = e_{11} = e_3$ which is what we intended to prove.
  \item If rule \textsc{EIf} was used, then we get the derivation
    $\eval{\true}{e'}$ for some $e'$ from the premise of the rule. However this
    is impossible because no rule evaluates $\true$ further.
  \end{enumerate}
\end{proof}
