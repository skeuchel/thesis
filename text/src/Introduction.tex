...
verify correctness of their proofs mechanically by \emph{automated theorem
  provers} (ATP). ATPs have also seen successful use in other fields of computer
science such as formal verification of programs and hardware. Theorem provers
may find proofs for theorems fully automatically or require a human user to
input the proof or proof hints in which case the system is also commonly called
a \emph{proof assistant}. In both cases, every reasoning step of the proof is
verified to be valid in the underlying logic. This has the benefit that gaps in
the proof are ruled out and unproven assumptions need to be explicitly
documented. This greatly increases the confidence in the validity of
(meta-theory) proofs.
...
assistant which offers a competitive degree of automation among proof
assistants.

To illustrate the benefits of mechanisations, consider the elaborate bug hunting
study of \cite{yang2011bugs} using their randomized test-case generator
...
specification of the x3C programming language. \cite{yang2011bugs} write the
...
This ideal view is unrealistic. Features may have dependencies between them or
may interact with each other even through they seem to be orthogonal. This is
more pronounced when features have side effects.
...
substitutable we call it a \emph{variable}. The operational semantics of such
languages often implement reduction of language construction with \emph{variable
  binding} by means of substitution.
...
Other kinds of \emph{generic functionality} that can be reused in this ways are,
for example, term equality and (first-order) unification.

\section{Overview}

This thesis is split into two parts corresponding to the two discussed means of
achieving reuse.

\paragraph{Modularity}
Part \ref{part:mod} deals with the modular composition of
language features. Chapter \ref{ch:modbackground} presents the necessary
background for this part. While modular development of software is a
well-studied topic in computer science, modular composition of proofs is not as
well-studied. Section \ref{sec:mod:reasoningalacarte} of the background chapter
introduces induction on extensible Church encodings as one way of achieving
proof modularity. Chapter \ref{ch:modpreduniv} develops an alternative way using
encodings based on modular predicative universes. This chapter contains the
material from
...
Side-effecting language features have a lot of interaction between them which
hinders modularization. This is a problem that is tackled in Chapter
\ref{ch:modmoneff} by developing a monadic denotational semantics
for features with side-effects that can be modularized. This chapter is based
on the publication
...
\noindent and also draws ideas from
...
Part \ref{part:gen} deals with the development of a generic substitution
operation and associated theorems. The solution put forward in this thesis is a
framework consisting of a specification language \Knot for abstract syntax with
variable binding and the tool \Needle that compiles \Knot-specifications to Coq
code. Chapters \label{ch:knotsyntax} and \label{ch:knotsemantics} present the
syntax and semantics of \Knot respectively while Chapters \ref{ch:elaboration}
and \ref{ch:codegen} discuss the elaboration and code generation underlying
\Needle.
...
Related to the content of this part is the article
...
\noindent which aims to develop a richer specification language that includes
variable binding constructs of realistic programming languages that \Knot does
not support.

Furthermore, the experiences gained from the work on \Needle \& \Knot
