...
basis for Chapter \ref{ch:modpreduniv} and \ref{ch:modmoneff}. In particular, we
summarize techniques that are used in the \emph{Meta-Theory \`a la Carte} (MTC)
framework, an existing solution for modular reasoning about programming
languages in Coq, on which we build. However, we limit ourselves to parts that
are relevant to Chapters \ref{ch:modpreduniv} and \ref{ch:modmoneff} and refer
the reader to the original paper~\cite{mtc} for full details.
