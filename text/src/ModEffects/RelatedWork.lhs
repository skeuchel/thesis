...
Since \citet{Moggi89a} first proposed monads to model side-effects, and
\citet{Wadler92a} popularized them in the context of Haskell, various
...
\citet{Filinski:LayeredMonads}'s layered monads and ~\citet{monatron}'s
Monatron.
...
laws for exceptions, for example, have been presented by \citet{levy06monads}.

A more recent model of side effects are effect handlers. They were introduced by
\citet{handlers} as a generalization from exception handlers to handlers for a
range of computational effects, such as I/O, state, and nondeterminism.
\citet{eff} built the language \emph{Eff} around effect handlers and show how to
implement a wide range of effects in it. \citet{hia} showed that effect handlers
can be implemented in terms of delimited continuations or free monads.
...
\citet{msos} modularizes structural operational semantics by means of a label
transition system where extensible labels capture effects like state and abrupt
termination. \citet{swierstra2008dtc} presents modular syntax with functor
coproducts and modular semantics with algebra compositions. To support effects,
he uses modular syntax to define a free monad. The effectful semantics for this
free monad is not given in a modular manner, however.
\citet{jaskelioff11modularity} present a modular approach for operational
...
Both \citet{schrijvers10zipper+tr} and %later
\citet{bahr11compositional} have shown how to define modular
...
\citet{voigt09free} has shown how to derive parametricity theorems for type
constructor classes such as |Monad|. Unfortunately, the reasoning power of
parametricity is limited, and parametricity is not supported by proof-assistants
like Coq.

A second technique uses \emph{algebraic laws}. \citet{liang96modular} present
one of the earliest examples of using algebraic laws for reasoning. They use
algebraic laws for reader monads to prove correctness properties about a modular
compiler. In contrast to our work, their compiler correctness proofs are
pen-and-paper and thus more informal than our proofs. Since they are not
restricted by a termination checker or the use of positive types only, they
exploit features like general recursion in their definitions.
\citet{effectiveadvice} have also used algebraic laws for the state monad, in
combination with parametricity, for modular proofs of non-interference of
aspect-oriented advice. \citet{justdoit} discuss several other algebraic laws
for various types of monads. However, as far as we know, we are the first to
provide an extensive mechanized library for monads and algebraic laws in Coq.
% \BO{Harrison's thesis? More? This is the most important line of related work!}

\subsection{Mechanization of Monad Transformers}
\citet{huffmann:transformers} illustrates an approach for mechanizing type
constructor classes in Isabelle/HOL with monad transformers. He considers
